        ...
        void OnUnitDeath(Unit* unit) override
        {
            ...
            if (isDragonspireRuneHallEncounterStartedButUnfinished(eventFlags))
            {
                if(Creature* creatureThatDied = unit->ToCreature())
                    if (const CreatureData* data = creatureThatDied->GetCreatureData()) //some creatures don't have a creature data
                    {
                        if (std::find(DragonspireMobs, DragonspireMobs + 3, data->id) != (DragonspireMobs + 3)) //if the creature was a DragonSpire Rune Hall Creature we're interested
